...
public class DocGraphDuplicatorTest extends GraphBITest {
	...
	public void testDocGraphDuplicationResult() {
		...
		int n, m;
		try (Transaction tx = toGraphDB.beginTx()) {
			n = IteratorUtil.count(GlobalGraphOperations.at(toGraphDB)
					.getAllNodes());
			m = IteratorUtil.count(GlobalGraphOperations.at(toGraphDB)
					.getAllRelationships());
			tx.success();
		}
		...
		try (Transaction tx = toGraphDB.beginTx()) {
			...
			Assert.assertEquals(1, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_Q001).iterator()));
			Assert.assertEquals(1, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_Q002).iterator()));
			Assert.assertEquals(1, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_S001).iterator()));
			Assert.assertEquals(1, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_S002).iterator()));
			Assert.assertEquals(1, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_I001).iterator()));
			Assert.assertEquals(1, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_I002).iterator()));
			Assert.assertEquals(1, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_I003).iterator()));
			Assert.assertEquals(
					1,
					IteratorUtil.count(nodeIndex.get(RDB2GRAPH_ID,
			...
			Assert.assertEquals(2, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_TOM).iterator()));
			Assert.assertEquals(2, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_JOHN).iterator()));
			Assert.assertEquals(2, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_EXPENSE).iterator()));
			Assert.assertEquals(2, IteratorUtil.count(nodeIndex.get(
					RDB2GRAPH_ID, KEY_REVENUE).iterator()));
